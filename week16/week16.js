...
function makeOne() {
	const formsOne = document.forms.length;
	paragraph.textContent = `Количество форм: ${formsOne}`;
}
...
function makeFour() {
	...
	for (let i = 0; i < document.forms.length; i += 1) {
		...
		const Name = formName.getAttribute('name');
		formNames.push(Name);

	}
	const stringNames = formNames.join(', ');
	paragraphFour.textContent = stringNames;
}
...
function makeFive() {
	...
	paragraphFive.textContent = ThirdForm.elements.length;
}
...
function makeSix() {
	...
	paragraphSix.textContent = SecondForm.elements.length;
}
...
function makeSeven() {
	const allForms = document.forms;
	const formElements = allForms[1].elements;
	const elementsList = [];
...
function makeEight() {
	...
	let nameElements = form.elements;
	for (let i = 0; i < form.length; i += 1) {
		let element = nameElements[i];
		const listItem = document.createElement('li');
		listItem.textContent = element.getAttribute("name");
		paragraphEight.appendChild(listItem);
	}
}
...
function makeNine() {
	...
	for (let i = 0; i < form.length; i += 1) {
		...
		const listItem = document.createElement('li');
		listItem.textContent = element.getAttribute("name");
		paragraphNine.appendChild(listItem);
	}
}
...
function makeTen() {
	...
	const input = form4.elements.fourthName;
...
function makeTwelve() {
	...
	const checkbox1 = form2.elements.checkboxOne.id;
	const checkbox2 = form2.elements.checkboxTwo.id;
	const checkbox3 = form2.elements.checkboxThree.id;

	paragraphTwelve.textContent = `id всех чекбоксов второй формы: ${checkbox1}, ${checkbox2}, ${checkbox3}`;
}
...
function checkButton(e) {
	const form4 = document.forms.lastForm;
	const button = form4.elements.fourthName;
	if (button.checked) {
...
function makeSixteen() {
	const form2 = document.forms.formTwo;
	const checkbox = form2.elements.checkboxThree;
...
formOne.addEventListener('submit', function (event) {
	...
	if ((Name.value === "") || (Email.value === "")) {
		document.getElementById("errorMessage").textContent = "Заполните все поля перед отправкой!"
	}
});
...
document.querySelector('.b-22').onclick = function (event) {
	...
	if (nameInput.value.trim() === '') {
		event.preventDefault();
		document.getElementById('result22').textContent = 'Укажите имя!';
	} else {
...
document.querySelector('.b-13').disabled = true;
const form4 = document.forms.lastForm;
const button = form4.elements.fourthName;
...
document.querySelector('.b-25').onclick = function () {
	...
	inputs.forEach(function (input) {
		...
		input.addEventListener('blur', function () {
			///input.remove(style.borderColor);
		});
	});
};
...
document.querySelector('.b-26').onclick = function (event) {
	event.preventDefault();
	const nameInput = document.forms.formThree.elements.thirdName;
	document.getElementById('result26').textContent = nameInput.value;
};
...
selectFormThree.addEventListener('change', function () {
	if (selectFormThree.selectedIndex >= 0) {
		document.getElementById('result28').textContent = "Опция выбрана";
	}
	else {
		document.getElementById('result28').textContent = "";
	}
});
...
function handleSubmit(event) {
	event.preventDefault();
	const Name = document.forms.formTwo.secondName;
	const Email = document.forms.formTwo.secondEmail;
